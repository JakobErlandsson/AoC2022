...
def get_neighbours(size):
    ...
    for y in range(1, size[0]-1):
        for x in range(1, size[1]-1):
            ...
            right = [(i, y) for i in range(x+1, size[0])]
            up = [(x, j) for j in range(y)]
            down = [(x, j) for j in range(y+1, size[1])]
...
class Solution:
    ...
    def __init__(self):
        ...
        self.size = (len(trees), len(trees))
        self.tree_map = {(x, y): trees[y][x] for x in range(self.size[0]) for y in range(self.size[1])}
        self.neighbours = {}
    ...
    def look_from_outside(self):
        ...
        for y in range(self.size[0]):
            for x in range(self.size[1]):
                if y in (0, self.size[0]-1) or x in (0, self.size[1]-1):
                    seen += 1
                else:
    ...
    def find_scenic_score(self, x, y):
        ...
        for i in range(y+1, self.size[1]):
            seen += 1
            tree = self.tree_map[(x, i)]
            if tree >= height:
                break
...
if __name__ == '__main__':
    ...
    match (getenv('part')):
        ...
        case 'part2':
            scores = [sol.find_scenic_score(x, y) for y in range(1, sol.size[0]-1) for x in range(1, sol.size[1]-1)]
            print(max(scores))
